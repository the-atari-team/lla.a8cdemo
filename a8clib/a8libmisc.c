...
word WaitKCX(byte bI)
{
    ...
    while((OS.ch == KEY_NONE) && (GTIA_READ.consol == CONSOL_NONE) && (OS.helpfg == 0)) {}

    // Grab the register values
    bK = OS.ch; // PEEK(OS.ch /*KEYPCH*/);
    bC = GTIA_READ.consol; // PEEK(CONSOL);
    bH = OS.helpfg; // PEEK(HELPFG);
    ...
    // Toggle CAPS
        ...
        bU = OS.shflok; // PEEK(SHFLOK);
        bU ^= 64;
        OS.shflok = bU; // POKE(SHFLOK, bU);
    }
    // Toggle Inverse
    else if (bK == KEY_INVERSE) {
        cR = bK;

        // Toggle if allowed
        if (bI == WON) {
            // Get current reg value, flip it, and put it back.
            bU = OS.invflg; // PEEK(INVFLG);
            bU ^= 128;
            OS.invflg = bU; // POKE(INVFLG, bU);
        }
    }
    // All else
    else {
    ...
    // Debounce key
    OS.ch = KEY_NONE; // POKE(KEYPCH, KNONE);
